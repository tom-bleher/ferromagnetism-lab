...
def digital_multimeter_resistance(R: float, *, include_lsd: bool = True) -> float:
    ...
    σ = (%rdg)·|R| + (%range)·range, optionally ⊕ LSD/√12.  Disable
    ``include_lsd`` when the logged value is already rounded and the
    display quantisation would be double-counted.
    """
    rng = next(r for r in sorted(_DMM_R_RANGES) if abs(R) <= r)
    pct_rdg, pct_rng = _DMM_R_RANGES[rng]
    sigma = pct_rdg * abs(R) + pct_rng * rng
    if include_lsd:
...
def oscilloscope_dual_cursor(V, *, gain_frac: float = 0.024,
    ...
    """Keysight DSO7012A dual-cursor ΔV σ: √((gain·|V|)² + floor² ⊕ LSD/√12)."""
