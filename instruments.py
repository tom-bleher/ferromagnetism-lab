...
def _manufacturer_spec_uncertainty(bound):
...
def oscilloscope_dual_cursor(V, *, gain_frac: float = 0.024,
    ...
    The manufacturer's dual-cursor ``±(gain·full-scale)`` accuracy is used
    directly as the instrument uncertainty and combined with display
    resolution.
    """
    V_arr = np.asarray(V, dtype=float)
    base = _manufacturer_spec_uncertainty(gain_frac * np.abs(V_arr))
